...
def test_run_validation_silent(caplog):
    options = ValidationOptions(files=[EXAMPLE], silent=True, version="2.0")
    results = run_validation(options)
    print_results(results)
    assert caplog.text == ''
...
def test_validate_file_warning(caplog):
    results = validate_file(IDENTITY_CUSTOM, ValidationOptions(version="2.0"))
    assert results.is_valid
...
def test_validate_file_invalid_brace(caplog):
    results = validate_file(INVALID_BRACES, ValidationOptions(version="2.0"))
...
def test_validate_file_invalid_comma(caplog):
    results = validate_file(INVALID_COMMA, ValidationOptions(version="2.0"))
    assert not results.is_valid
...
def test_validate_file_invalid_missing_modified(caplog):
    results = validate_file(INVALID_IDENTITY, ValidationOptions(version="2.0"))
    assert not results.is_valid
...
def test_run_validation_stdin(monkeypatch):
    ...
    options = ValidationOptions(files=sys.stdin, version="2.0")
    results = run_validation(options)
    assert results[0].is_valid
