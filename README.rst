...
If your STIX is already in a Python dictionary (for example if you have already run `json.loads()`), use `schema_validate()` instead:

.. code:: python

  import json
  from stix2validator import schema_validate, print_results

  stix_json_string = "..."
  stix_obj = json.loads(stix_json_string)
  results = schema_validate(stix_obj)
  if results.is_valid:
      print_results(results)

You can pass a ValidationOptions object into `validate_file()`, `validate_string()`, or `schema_validate()` if you want behavior other than the default:
